...
// Collapsed sidebar state, persisted across reloads (mirrors the theme store's
// localStorage approach). Naive's sider animates the width change for us.
const COLLAPSE_KEY = 'archivarr-sidebar-collapsed'
...
// renderIcon wraps an icon component for use in n-menu options, so the collapsed
// rail shows a recognizable glyph rather than a blank row.
...
const menuOptions = [
  { label: 'Dashboard', key: '/', icon: renderIcon(GridOutline) },
  { label: 'Media', key: '/media', icon: renderIcon(FilmOutline) },
  { label: 'Drives', key: '/drives', icon: renderIcon(ServerOutline) },
  { label: 'Activity', key: '/activity', icon: renderIcon(PulseOutline) },
  { label: 'Recovery', key: '/recovery', icon: renderIcon(ArrowUndoOutline) },
...
<template>
  <n-layout has-sider style="height: 100vh">
    <n-layout-sider bordered collapse-mode="width" :width="220" :collapsed-width="64" :collapsed="collapsed"
      show-trigger="arrow-circle" :native-scrollbar="false" content-style="padding-top: 12px"
      @update:collapsed="onCollapse">
      <div class="brand" :class="{ 'brand--collapsed': collapsed }">
        <img src="/favicon.svg" alt="" class="brand-mark" />
        <span v-if="!collapsed" class="brand-text">Archi<span class="brand-accent">varr</span></span>
      </div>
      <n-menu :value="selectedKey" :options="menuOptions" :collapsed="collapsed" :collapsed-width="64"
        :collapsed-icon-size="22" @update:value="onMenu" />
    </n-layout-sider>
    ...
    <n-layout>
        ...
        <div class="header-right">
          ...
          <n-tooltip>
            <template #trigger>
              <n-button size="small" tertiary circle
                :aria-label="theme.isDark ? 'Switch to light mode' : 'Switch to dark mode'" @click="theme.toggle()">
                <template #icon>
                  <n-icon>
          ...
          <n-dropdown trigger="click" :options="userOptions" @select="onUser">
            <n-button size="small" tertiary>
              <template #icon>
                <n-icon><svg viewBox="0 0 24 24" width="1em" height="1em">
                    <path fill="currentColor"
                      d="M12 12a5 5 0 1 0 0-10a5 5 0 0 0 0 10m0 2c-4.42 0-8 2.69-8 6v2h16v-2c0-3.31-3.58-6-8-6" />
                  </svg></n-icon>
              </template>
      ...
      <n-layout-content content-style="padding: 24px" :native-scrollbar="false">
        <n-alert v-if="automation.paused" type="warning" style="margin-bottom: 16px" title="Automation is paused">
